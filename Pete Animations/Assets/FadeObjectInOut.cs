    ...
    IEnumerator FadeSequence(float fadingOutTime)
    {
        ...
        if (colors == null)
        {
            ...
            for (int i = 0; i < rendererObjects.Length; i++)
            {
                for (int j = 0; j < rendererObjects[i].materials.Length; j++)
                {
                    colors[index] = rendererObjects[i].materials[j].color;
                    index++;
                }
            }
        }
        ...
        while ((alphaValue >= 0.0f && fadingOut) || (alphaValue <= 1.0f && !fadingOut))
        {
            ...
            for (int i = 0; i < rendererObjects.Length; i++)
            {
                for (int j = 0; j < rendererObjects[i].materials.Length; j++)
                {
                        Color newColor = (colors != null ? colors[index] : rendererObjects[i].materials[j].color);
                    if (fadingOut)
                    ...
                    rendererObjects[i].materials[j].SetColor("_Color", newColor);
                    index++;
                }
            }
